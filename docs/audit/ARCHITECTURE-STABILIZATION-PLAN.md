...
| **AC-2** | 57 raw `fetch(${BASE}/…)` calls in 18 view files bypass `bget`/`bpost` wrappers. | `frontend/src/lib/billing.ts` (`bget`/`bpost`/etc.) | 18 view files | **H** | count of raw `fetch(` in views | ⏸ DEFERRED — chained `.then(r => r.ok ? r.json() : null)` pattern; folded into DF-1/DF-2 useFetch wave so the hook rewrites it once. |
...
| **SM-1** | `token` prop-drilled to all 55+ views. | `AuthContext` (React context) | All 55+ view files | **H** | grep `token: string` → 0 in views | ⏸ DEFERRED — full AuthContext refactor (token + capabilities + session) folded into DF-1/DF-2 wave so the new hook consumes context from day one. |
...
| **DF-1** | `useEffect + fetch + setState + alive guard` pattern — no server-state caching. | `useFetch<T>(url, token)` hook or TanStack Query | All views with direct `fetch` + ~35 via `bget` | **H** | alive guard count → 0 | ⬜ TODO — Phase 2 Part 2. |
| **DF-2** | `let alive = true; ...` cleanup idiom duplicated 78 times. | Encapsulate in `useFetch<T>` hook | ~20 files, 78 pairs | **H** | grep `let alive` → 0 | ⬜ TODO — Phase 2 Part 2 (paired with DF-1). |
| **DF-8** | No OpenAPI-to-TS type generation. | `openapi-typescript` codegen | ~60 type files | **H** | typecheck with generated types | ⬜ TODO — Phase 2 Part 2 scaffolding. |
...
| Phase 2 — API + State | 11 | 6 | 4 | 1 | **8 done · 3 deferred to Part 2** | **3** |
