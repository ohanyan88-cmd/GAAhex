# GAAhex™ — Brand Master Specification v1.1
...
> **v1.1 brand-law change — connector lines removed.** All connector / mesh / network / decorative line systems are deprecated and removed from the entire brand system. The mark now consists only of hexagon outline cells, the gold destination hexagon, and the wordmark. **The network is represented by the spatial relationship of the hexagonal nodes themselves. No explicit connector lines are used.** Geometry, spacing, colors, chevron direction, the gold destination node, and the wordmark are unchanged.
...
**Asset 79 — Partner lockup (equal weight).** SVG template `gaahex-cobrand-partner-template.svg`. Two logos at equal optical height separated by a vertical hairline divider (`#E2E5EA`); GAAhex left by default; gap = 2× the smaller logo's cap height.

---

# 23. Co-Branding Rules

- Never merge or fuse marks; always separate by clear space or a divider.
