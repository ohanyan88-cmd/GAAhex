...
- **Tenant-filter CI gate (`backend/scripts/check_tenant_filter.py`)** — RESOLVED 2026-06-05. Six call sites in `auth.py`, `billing_product.py`, `portal_auth.py`, `payment_allocation.py`, and `product_versions.py` were flagged as missing explicit Python-side `tenant_id` filters. All six were proven safe via the RLS + tenant-provenance chain (RLS-bound session + tenant-internal IDs validated upstream) and annotated with `# noqa: tenant-filter` plus an inline rationale on the query-starter line. Commit `87bb42c`; no runtime behaviour change. CI run **27031511102** (2026-06-05) returned `success` — the first green run on `main` since this gate was introduced. See `docs/audit/POST-D19-STABILIZATION-COMPLETE-2026-06-05.md` for the milestone note.

- **Main-branch CI baseline restored** — 2026-06-05. After the D19 Path A reconciliation (`46f25d0`) + the tenant-filter remediation (`87bb42c`), CI run `27031511102` is the **first green run on `main`** since 2026-06-04 (8+ consecutive red runs before that). Blocking gates passing: Frontend (tsc + npm audit), Backend (pytest + ruff + pip-audit), Secret scan (gitleaks). The `backend-rls` job remains red as expected — `continue-on-error: true` per `ci.yml:141`, tracked as **TD13** (open, scoped for M1).
