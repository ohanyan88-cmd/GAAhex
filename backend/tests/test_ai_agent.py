...
def test_validate_move_lead_requires_name_and_valid_status():
    action, clean = _validate_action("move_lead", {"lead_name": "A", "to_status": "qualified"})
    assert action == "move_lead" and clean == {"lead_name": "A", "to_status": "QUALIFIED"}   # normalized upper
    with pytest.raises(HTTPException) as e1:
        _validate_action("move_lead", {"to_status": "QUALIFIED"})            # missing lead_name
...
async def test_act_move_nonexistent_lead_404(client, admin):
    r = await client.post("/api/ai/act", headers=admin, json={
        "action": "move_lead",
        "args": {"lead_name": f"Ghost {uuid.uuid4().hex}", "to_status": "QUALIFIED"}})
