...
async def _own_ticket(s: AsyncSession, cu: CustomerUser, ticket_id: uuid.UUID) -> HelpdeskTicket:
    t = (await s.execute(
        select(HelpdeskTicket).where(  # tenant-filter-ok: cross-tenant — customer-portal; explicit customer_id ownership check
...
async def list_tickets(
    ...
    tickets = (await s.execute(
        select(HelpdeskTicket).where(HelpdeskTicket.customer_id == cu.customer_id)  # tenant-filter-ok: cross-tenant — customer-portal; scoped by cu.customer_id (auth-bound)
...
async def get_ticket(
    ...
    replies = (await s.execute(
        select(PortalTicketReply).where(  # tenant-filter-ok: cross-tenant — ticket ownership verified by _own_ticket above
