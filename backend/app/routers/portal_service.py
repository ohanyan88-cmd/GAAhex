...
async def list_services(
    ...
    services = (await s.execute(
        select(Service).where(Service.customer_id == cu.customer_id)  # tenant-filter-ok: cross-tenant — customer-portal; scoped by cu.customer_id (auth-bound)
...
async def list_subscriptions(
    ...
    subs = (await s.execute(
        select(Subscription).where(Subscription.customer_id == cu.customer_id)  # tenant-filter-ok: cross-tenant — customer-portal; scoped by cu.customer_id (auth-bound)
...
async def list_usage(
    ...
    sub_ids = (await s.execute(
        select(Subscription.id).where(Subscription.customer_id == cu.customer_id)  # tenant-filter-ok: cross-tenant — customer-portal; scoped by cu.customer_id
    )).scalars().all()

    if not sub_ids:
        return []

    q = select(UsageRecord).where(UsageRecord.subscription_id.in_(sub_ids))  # tenant-filter-ok: cross-tenant — usage scoped to customer's own subscription ids (computed above)
