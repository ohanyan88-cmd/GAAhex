...
async def invoice_document(invoice_id: uuid.UUID, user: User = Depends(current_user), s: AsyncSession = Depends(get_session)):
    ...
    issuer = _e(tenant.name if tenant else "GAAex")
    ...
    {payments_block}
    <div class="foot">Generated by {issuer} · GAAex · All amounts in Armenian Dram (֏).</div>
    """
...
async def payment_receipt(
    ...
    <div class="foot">
      Confirmed by {method_label}{provider_detail} · {issuer} · GAAex · All amounts in Armenian Dram (֏).
    </div>
    """
