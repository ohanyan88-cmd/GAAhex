...
async def _resources(s, service_id) -> list[ServiceResource]:
    return list((await s.execute(
        select(ServiceResource).where(ServiceResource.service_id == service_id).order_by(ServiceResource.created_at)  # tenant-filter-ok: cross-tenant — helper; caller validates service tenant via _get_service
    )).scalars().all())
