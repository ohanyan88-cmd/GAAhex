...
from .seed_dev_bulk import seed_dev_bulk_if_empty, seed_dev_extras_if_empty, seed_dev_threads_if_empty, seed_dev_pipeline_if_empty, seed_dev_notifications_if_empty, seed_dev_people_content_if_empty, _dev_seed_enabled
...
async def lifespan(app: FastAPI):
    ...
    if _dev_seed_enabled():
        ...
        try:
            await seed_dev_extras_if_empty()  # demo CalendarEvents + Communications tied to dev_bulk customers (idempotent)
        except Exception:
        ...
        try:
            await seed_dev_notifications_if_empty()  # demo notifications for the bell + Notifications page (idempotent)
        except Exception:
            import logging as _lg
            _lg.getLogger("gaahex").exception("seed_dev_notifications_if_empty failed (non-fatal — continuing boot)")
        try:
            await seed_dev_people_content_if_empty()  # demo Benefits + Knowledge Base content for My Profile (idempotent)
        except Exception:
            import logging as _lg
            _lg.getLogger("gaahex").exception("seed_dev_people_content_if_empty failed (non-fatal — continuing boot)")
...
app.include_router(mail.router)                  # /api/mail/* — Mail module (per-tenant email client); fixed path, before generic records
app.include_router(messaging.router)             # /api/messaging/* — per-tenant SMS/Telegram/WhatsApp channels
...
app.include_router(workspace_home.router)            # /api/workspace?role= (My Work data payload; before records)
