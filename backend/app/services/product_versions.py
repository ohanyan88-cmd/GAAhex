...
async def current_version_for(
    ...
    rows = (await session.execute(
        select(ProductVersion).where(ProductVersion.product_id == product_id)  # tenant-filter-ok: — internal service; caller passes RLS-bound `session` and tenant-validated product_id; ProductVersion.tenant_id enforced by RLS.
