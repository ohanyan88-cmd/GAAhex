...
# (table, tenant key column). RLS keys on this column matching the gaahex.tenant_id GUC.
...
# NOTE: deliberately NO `FORCE ROW LEVEL SECURITY`. The table owner / superuser (the `gaahex` role
...
# NOSUPERUSER `gaahex_app` role (created below); the isolation proof test exercises that role.
...
def upgrade() -> None:
    ...
    op.execute("""
        ...
        END $$;
    """)
    op.execute("GRANT USAGE ON SCHEMA public TO gaahex_app;")
    op.execute("GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO gaahex_app;")
    op.execute("GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO gaahex_app;")
    op.execute("ALTER DEFAULT PRIVILEGES IN SCHEMA public "
               "GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO gaahex_app;")
    ...
    for table, key in _TABLES:
        ...
        op.execute(f"""
            CREATE POLICY tenant_isolation ON {table}
              USING ({key} = NULLIF(current_setting('gaahex.tenant_id', true), '')::uuid)
              WITH CHECK ({key} = NULLIF(current_setting('gaahex.tenant_id', true), '')::uuid);
        """)


def downgrade() -> None:
    # Drop the policies + disable RLS. We deliberately DO NOT drop the `gaahex_app` role here: it is a
