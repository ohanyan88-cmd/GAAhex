...
> **Blueprint:** the full architecture lives in `../GAAhex-Vision/` (read its `README.md`).
> This repo is the build. Current milestone: **M0** (see `../GAAhex-Vision/6-platform-delivery/31-phase-0-scope.md`).
...
Postgres → `localhost:5433` (user/pass/db = `gaahex`). Redis → `localhost:6380`.
...
CRUD from config. See `../GAAhex-Vision/6-platform-delivery/31-phase-0-scope.md`.
