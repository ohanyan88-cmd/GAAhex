...
export default function InvoicesView({
  ...
  async function runDunning() {
    try {
      ...
      toast.error(
        err.status === 404 ? t('invoices.dunning.na', "Dunning isn't available yet") : err.message,
      )
    }
  }
  ...
  async function runCycle() {
    ...
    setCycleBusy(true)
    try {
      const r = await bpost<{ generated?: number; skipped?: number }>(
        token!,
        '/api/billing/run-cycle',
      )
      const msg = t(
        'billing.cycleResult',
        'Billing cycle: {generated} generated, {skipped} skipped',
      )
        .replace('{generated}', String(r?.generated ?? 0))
      ...
      if (err.status === 404) {
        setCycleNA(true)
        toast.error(t('billing.cycleNA', "Billing cycle isn't available yet"))
  ...
  const cust = (inv: Invoice) =>
    inv.customer_id ? (names[inv.customer_id] ?? humanRef({ id: inv.customer_id })) : '—'
  ...
  const outstanding = all
    .filter((i) => INVOICE_OUTSTANDING.includes((i.status ?? '').toUpperCase() as any))
  ...
  const kpis: KPISpec[] =
    all.length > 0
      ? [
          {
            label: t('invoices.kpi.totalBilled', 'Total billed'),
          ...
          {
            label: t('invoices.kpi.paid', 'Paid'),
            value: paidCount,
            ...
            ? [
                {
                  ...
                  value: overdueCount,
                  subtitle: t('invoices.kpi.overdue.sub', 'action required'),
                  danger: true,
  ...
  return (
    <PageShell
      ...
      secondaryActions={[
        ...(canEditInvoice
          ? [{ label: t('invoices.action.runDunning', 'Run dunning'), onClick: runDunning }]
          : []),
        ...(canConfigure && canEditInvoice && !cycleNA
          ? [
              {
                label: cycleBusy
                  ? t('billing.running', 'Running…')
                  : t('billing.runCycle', 'Run billing cycle'),
                onClick: runCycle,
                disabled: cycleBusy,
              },
            ]
    ...
    >
      ...
      {unavailable && (
        <EmptyState
          ...
          title={t('invoices.unavailable.title', "Billing isn't available yet")}
          message={t(
            'invoices.unavailable.msg',
            'Invoices will appear here once the billing service is enabled.',
          )}
        />
      )}
      {list && !unavailable && list.length === 0 && !error && (
        <EmptyState
          ...
          message={t('invoices.empty.msg', 'No invoices match this filter.')}
        />
      )}
          ...
          <div className="grid-wrap">
            <table className="grid">
              <thead>
                <tr>
                  {cfg.columns.map((c) => (
                    <th key={c.key} scope="col" className={COL_CLASS[c.key] ?? ''}>
                      {c.label}
                    </th>
                  ))}
                  {cf.headers()}
              ...
              <tbody>
                ...
                {list.length === 0 && (
                  <tr>
                    <td
                      colSpan={cfg.columns.length + 1 + cfg.customFields.length}
                      style={{ padding: 0 }}
                    >
                      <EmptyState
