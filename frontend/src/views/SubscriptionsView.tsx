...
import { Plus, ChevronsUpDown, ArrowUp, ArrowDown } from 'lucide-react'
...
export default function SubscriptionsView({
  ...
  const cust = (s: Subscription) =>
    s.customer_id ? (names[s.customer_id] ?? humanRef({ id: s.customer_id })) : '—'
  ...
  async function rateUsage(id: string) {
    try {
      ...
      const inv =
        res?.invoice_number ??
        res?.number ??
        (res?.invoice_id ? humanRef({ id: String(res.invoice_id) }) : '')
      const total = res?.total
      toast.success(
        `Usage rated${inv ? ` → ${inv}` : ''}${typeof total === 'number' ? ` (${money(total)})` : ''}`,
      )
    } catch (e) {
  ...
  function renderCell(colKey: string, s: Subscription) {
    switch (colKey) {
      case 'customer':
        return cust(s)
      case 'plan':
        return s.plan_name ? <span className="mono">{s.plan_name}</span> : <span>—</span>
      case 'cycle':
        return s.cycle ? (
  ...
  return (
    <PageShell
      ...
      secondaryActions={
        !unavailable && canConfigure && onConfigure
      ...
      <ErrorBoundary onReset={load}>
        ...
        {unavailable && (
          <EmptyState
            ...
            message="Subscriptions will appear here once the billing service is enabled."
          />
        )}
        {list && !unavailable && list.length === 0 && !error && (
          <EmptyState
            ...
            message="Subscriptions you create will show up here."
          />
        )}
              ...
              <table className="grid">
                <thead>
                  <tr>
                    ...
                    {cf.headers()}
                    <th scope="col" className="actions-col" aria-label="Actions">
                          ...
                          <div className="row-actions" style={{ justifyContent: 'flex-end' }}>
                            {(() => {
                              ...
                              if (!canceled) {
                                actions.push({
                                  key: 'gen',
                                  label: 'Generate invoice for current period',
                                  icon: <ReceiptIcon size={14} />,
                                  onClick: () => generate(s.id),
                                })
                                actions.push({
                                  key: 'rate',
                                  label: 'Rate metered usage',
                                  icon: <GearIcon size={14} />,
                                  onClick: () => rateUsage(s.id),
                                })
                              }
                              if (st === 'ACTIVE') {
                                actions.push({
                              ...
                              if (st === 'ACTIVE' || st === 'SUSPENDED') {
                                actions.push({
                                  ...
                                  onClick: () => {
                                    if (
                                      window.confirm(
                                        `Cancel subscription "${s.plan_name ?? humanRef({ id: s.id })}"? This cannot be undone.`,
                                      )
                                    ) {
                                      action(s.id, 'cancel')
                                    }
