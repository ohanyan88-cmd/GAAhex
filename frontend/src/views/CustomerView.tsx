...
export default function CustomerView({
  ...
  async function loadAccountsAndBalances() {
    ...
    setBalanceFatal(false)
    setShowConsolidated(false)
    const ar = await bget<Account[]>(
      token!,
      `/api/accounts?customer=${encodeURIComponent(customerId)}`,
    )
    if (
      ar.status === 403 ||
      ar.status === 404 ||
    ...
    if (accts.length > 1) {
      ...
      const cr = await bget<ConsolidatedBalance>(
        token!,
        `/api/accounts/${root.id}/balance/consolidated`,
      )
  ...
  async function load() {
    ...
    if (!res.ok || !res.data) {
      setError(t('cust.loadError', 'Failed to load this customer'))
      return
    }
    const c = res.data
    setData(c)
    if (Array.isArray(c.services)) {
      ...
      const sv = await bget<Service[]>(
        token!,
        `/api/services?customer=${encodeURIComponent(customerId)}`,
      )
  ...
  useEffect(() => {
    loadTab(tab) /* eslint-disable-next-line react-hooks/exhaustive-deps */
  }, [tab])

  if (fatal === 'denied')
    return (
      <PermissionDenied
        message={t('cust.denied', "You don't have permission to view this customer.")}
      />
    )
  if (fatal === 'notfound')
    return (
      <NotFound
        what={t('cust.what', 'customer')}
        message={t('cust.notFoundMsg', 'This customer may have been moved, renamed, or deleted.')}
      />
    )

  const p = data?.profile
  const name = p?.name ?? p?.title ?? (p ? humanRef({ id: p.id }) : '')
  ...
  const statusSummary: StatusSummary | undefined = p?.status
    ...
    : undefined
  const subtitle = p?.id
    ? `${t('cust.subtitle', 'Customer 360')} · ${humanRef({ id: p.id })}`
    : t('cust.subtitle', 'Customer 360')
    ...
    >
      ...
      {!data && !error && (
        <>
          <div
            className="kpi-strip"
            style={{ marginBottom: 'var(--gx-space-20)' }}
            aria-busy="true"
          ...
          {[0, 1, 2].map((i) => (
            <div key={i}>
              <div
                className="kpi-tile-skeleton"
                style={{
                  height: 'var(--gx-space-7)',
                  width: 140,
                  margin: 'var(--gx-space-18) 0 var(--gx-space-5)',
                }}
              />
              <div className="card" style={{ padding: 'var(--gx-space-7)' }}>
                ...
                <div
                  className="kpi-tile-skeleton"
                  style={{ height: 'var(--gx-space-6)', width: '60%' }}
                />
              </div>
            </div>
          ))}
        ...
        <>
          ...
          <div className="kpi-strip" style={{ marginBottom: 'var(--gx-space-20)' }}>
            <KPITile
              ...
              ariaLabel={t(
                'cust.outstandingAria',
                'Outstanding amount. Click to see overdue invoices.',
              )}
            />
            <KPITile
