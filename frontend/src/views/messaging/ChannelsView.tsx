...
const CHANNELS: { value: ChannelKind; label: string; tokenLabel: string; senderLabel: string }[] = [
  {
    ...
    senderLabel: 'Default chat / @username',
  },
  {
    value: 'SMS',
    label: 'SMS (Viva Armenia)',
...
export default function ChannelsView() {
  ...
  async function save() {
    ...
    try {
      const payload: Partial<ChannelAccountInput> = {
        ...
        sender_id: draft.sender_id || undefined,
        config: draft.config,
      }
  ...
  async function test(a: ChannelAccount) {
    try {
      ...
      if (r.ok) toast.success(`${a.display_name}: connected`)
      else toast.error(`${a.display_name}: ${r.detail || r.status}`)
      await reload()
    } catch (e: any) {
  ...
  async function remove(a: ChannelAccount) {
    ...
    try {
      await api.deleteAccount(token!, a.id)
      await reload()
    } catch (e: any) {
  ...
  return (
    <PageShell
      type="CONFIGURATION"
      breadcrumb={['Settings', 'Messaging Channels']}
      icon={<MessageIcon size={18} />}
      title="Messaging Channels"
      subtitle="Per-tenant SMS, Telegram, and WhatsApp — each tenant sends from its own credentials."
      primaryAction={{ label: 'Add channel', icon: <PlusIcon size={15} />, onClick: openCreate }}
    >
      {accounts === null ? (
        <LoadingState />
      ) : error ? (
      ...
      {open && (
        <Modal
          ...
          title={draft.id ? 'Edit channel' : 'Add channel'}
          footer={
            <>
          ...
          <div className="gx-stack">
            ...
            <FormField label={meta.senderLabel}>
              <Input
                ...
                onChange={(e) => setDraft((d) => ({ ...d, sender_id: e.target.value }))}
              />
            </FormField>
            <FormField
              label={meta.tokenLabel}
              hint={draft.has_token ? '•••• set — type to replace' : undefined}
            >
              <Input
                type="password"
            ...
            {draft.channel === 'WHATSAPP' && (
              <FormField label="Phone number ID (WABA)">
                <Input
                  value={(draft.config?.phone_number_id as string) || ''}
                  onChange={(e) =>
                    setDraft((d) => ({
                      ...d,
                      config: { ...d.config, phone_number_id: e.target.value },
                    }))
                  }
                />
              </FormField>
