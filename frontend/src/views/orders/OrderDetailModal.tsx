...
export function OrderDetailModal({
    ...
    if (!res.ok) {
      setError(
        res.status === 404
          ...
          : t('orders.failedToLoad', 'Failed to load order'),
      )
      return
    }
    setOrder(res.data)
  ...
  const cust = order?.customer_id
    ? (customerNames[order.customer_id] ?? humanRef({ id: order.customer_id }))
    : '—'
  ...
  const drawerStatus =
    statusVariant && order?.status
      ? {
          ...
          variant: statusVariant as 'active' | 'degraded' | 'critical' | 'neutral' | 'info',
        }
      : undefined
  ...
  const fields: RecordDrawerField[] = order
    ? [
        ...
        {
          key: 'total',
          label: t('orders.col.total', 'Total'),
          value: <span className="mono tnum">{money(order.total)}</span>,
        },
        {
          key: 'created',
          label: t('orders.col.created', 'Created'),
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--gx-space-3)' }}>
                {order.items.map((it) => (
                  ...
                  >
                    ...
                    <span className="mono tnum" style={{ color: 'var(--gx-text-3)' }}>
                      ×{it.quantity}
                    </span>
                    <span className="mono tnum" style={{ minWidth: 64, textAlign: 'right' }}>
                      {money(it.line_total)}
                    </span>
                  </div>
                ))}
  ...
  return (
    <>
      <RecordDrawer
        ...
        onClose={onClose}
        entityKey="order"
        id={order ? order.number : id}
        title={
          order
            ? `${t('orders.detail.orderTitle', 'Order')} ${order.number}`
        ...
        footer={
          canEdit && order ? (
            <>
              {canFinalCancel && (
                <Button variant="ghost" size="sm" disabled={busy} onClick={() => action('cancel')}>
