...
This doc covers everything needed to safely run, back up, and recover a GAAhex Portal install.
...
- [ ] `GAAHEX_FIELD_KEY` is a real Fernet key (see below) — stored in vault, not just in `.env`
...
- [ ] Postgres `gaahex_app` user has minimal grants (SELECT/INSERT/UPDATE/DELETE on app tables only)
...
pg_dump -h HOST -p PORT -U gaahex -d gaahex -F c -Z 9 \
  -f "gaahex_$(date +%Y%m%d_%H%M%S).dump"
```
...
```bash
pg_restore -h HOST -p PORT -U gaahex -d gaahex_restore \
  --no-owner --no-privileges gaahex_20260101_030000.dump
```

Create the target DB first:
```bash
createdb -h HOST -p PORT -U gaahex gaahex_restore
```
...
psql -h HOST -p PORT -U gaahex -d gaahex_restore -c "
...
1. Create a fresh Postgres DB: `createdb -U gaahex gaahex_restore_test`
2. Restore the latest dump (section 2b).
3. Point a test app instance at `gaahex_restore_test`:
   ```
   DATABASE_URL=postgresql+asyncpg://gaahex:PASS@HOST:PORT/gaahex_restore_test
   ```
...
7. Drop the restore DB: `dropdb -U gaahex gaahex_restore_test`
...
Set `GAAHEX_FIELD_KEY=<output>` in `.env` and in your secrets vault.

### 5b. Key rotation procedure

When you need to rotate `GAAHEX_FIELD_KEY` (scheduled or after suspected compromise):
...
- [ ] Set `GAAHEX_TENANT_ID` in `.env` to the tenant UUID from `SELECT id FROM tenant LIMIT 1`
- [ ] Restart app
